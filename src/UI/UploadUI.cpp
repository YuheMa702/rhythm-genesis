...
//     int fileFontSize = static_cast<int>(screenHeight * 0.02f);
...
//         std::string filename = songNameBox->getText();

//         std::string outputPath = "../src/Game/json/" + filename + ".json";
...
UploadUI::UploadUI(sf::RenderWindow* window, const std::string& selectedFile)
    ...
    saveButton = new UIButton("Save", screenWidth * 0.1f, screenHeight * 0.6f,
        ...
        //std::string filename = musicPath.stem().string();
        std::string filename = songNameBox->getText();
        std::string extension = musicPath.extension().string();

        std::filesystem::create_directories("../src/Game/json/");
        std::filesystem::create_directories("../src/Game/music/");

        std::string beatmapOutputPath = "../src/Game/json/" + filename + ".json";
        std::ofstream outFile(beatmapOutputPath);
        if (outFile) {
            outFile << beatmapJSON;
            outFile.close();
            spdlog::info("Beatmap saved to {}", beatmapOutputPath);
        } else {
            spdlog::error("Failed to save beatmap to {}", beatmapOutputPath);
        }

        // Save the uploaded music file to music directory with correct extension
        std::filesystem::path destinationMusicPath = "../src/Game/music/" + filename + extension;
        try {
            std::filesystem::copy_file(musicPath, destinationMusicPath, std::filesystem::copy_options::overwrite_existing);
            spdlog::info("Music file copied to {}", destinationMusicPath.string());
        } catch (const std::exception& e) {
            spdlog::error("Failed to copy music file: {}", e.what());
        }
